...
# Query 9 : Countries with Transaction_count >= 2000
# retail 테이블들의 거래들을 Country 별로 묶고, 
# 각 국가의 거래수(transaction_count)를 계산한 뒤,
# 거래 수가 2000 이상인 국가만 남기고,
# transaction_count 기준 내림차순으로 결과를 변환해줘 !
...
# COUNT(*) = 그 그룹 안의 row (행) 의 개수를 세라 !
# 각 행 = 여기선 1회의 거래
# 예를 들어서, France 라는 항목이 Country 에 2번 나오면, Count(*) = 2 로 해당 항목 계산됨
# 2회 거래했다는 의미 !

result_9 = pd.read_sql_query(query_9, conn)
print(f"Query 9 : Countries with transaction_count >= 2000 \n{result_9}")
...
# Query 10 : Daily Sales
# retail 테이블들의 거래들을 InvoiceDate 의 날짜 기준으로 묶고,
# 날짜별 거래수 (transaction_count) 와 TotalPrice 합계 (daily_sales)를 계산해서,
# order_date 기준 오름차순으로 정렬하고 상위 15일을 반환해줘 !

query_10 = """
SELECT DATE(InvoiceDate) AS order_date,
       Count(*) AS transaction_count,
       SUM(TotalPrice) AS daily_sales
FROM retail
...
print(f"Query 10 : Daily Sales \n{result_10}")

# Query 11 : Monthly Sales
# retail 테이블들의 거래들을 InvoiceDate 의 연-월 기준으로 묶고,
# 월별 TotalPrice 합계(monthly_sales) 를 계산해서,
# order_month 기준 오름차순으로 정렬해서 반환해 !
...
query_11 = """
SELECT strftime('%Y-%m', InvoiceDate) AS order_month,
...
# strftime('%Y-%m', InvoiceDate) 함수는 날짜에서 연도와 월만 추출한다 !
# %Y → 4자리 연도 (2010)
# %m → 2자리 월 (12)
# 결과 : 2010-12
...
print(f"Query 11 : Monthly sales \n{result_11}")
